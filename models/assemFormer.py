...
class AssemFormer(BaseFormer):
    ...
    def __init__(
        ...
        super().__init__(InChannels*2, FfnMultiplier, NumAttnBlocks, AttnDropRate,
                         DropRate, FfnDropRate, PatchRes, Dilation, ViTSELayer, **kwargs)
        # AssembleFormer: input changed from just global to local + global
        self.ConvProj = BaseConv2d(2 * self.DimCNNOut, InChannels*2, 1, 1, BNorm=True)
    ...
    def forward(self, x):
        ...
        # print(self.Dropout(Fm).size())
        # AssembleFormer: skip connection
        return rgb + self.Dropout(Fm)[:, :c, :, :], ir + self.Dropout(Fm)[:, c:, :, :]
