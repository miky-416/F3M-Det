...
from models.common import MAM3, Conv, MAM1, MAM2, CEM1
...
class Fuse1(nn.Module):
    def __init__(self, in_channel):
        ...
        # self.ldc = LDConv(in_channel, in_channel, num_param=6, stride=1)
...
class Fuse12(nn.Module):
    def __init__(self, in_channel):
        ...
        # self.ff2 = FreqFusion(int(in_channel / 2), int(in_channel / 2))
        # self.dy = DynamicConv1(in_channel, in_channel, 3, 1, 1)
        # self.fus1 = Conv(in_channel * 2, in_channel, 1, 1, 0)
    def forward(self,x):
        rgb = x[0]
...
class FFuse11(nn.Module):
    def __init__(self, in_channel):
        super(FFuse11, self).__init__()
        self.mam = MAM2(in_channel)
        self.dda = DDA(in_channel)
        self.ff = FreqFusion1x(in_channel, in_channel)
    ...
    def forward(self, x):
        ...
        return [final, dda_gr, t]


class FFuse12(nn.Module):
    def __init__(self, in_channel):
        super(FFuse12, self).__init__()
        self.mam = MAM2(in_channel)
        self.dda = DDA(in_channel)
        self.conv_1x1rgb = nn.Conv2d(2 * in_channel, in_channel, kernel_size=1)
        self.conv_1x1ir = nn.Conv2d(2 * in_channel, in_channel, kernel_size=1)
        self.ff1 = FreqFusion1x(in_channel, in_channel)
        self.ff2rgb = FreqFusion2x(in_channel, in_channel)
        self.ff2ir = FreqFusion2x(in_channel, in_channel)
    ...
    def forward(self, x):
        ...
        y = [rgb, ir]
        gr = self.mam(y)
        dda_gr = self.dda(gr, ir)

        rgb_512_256 = self.conv_1x1rgb(rgb_512)
        ir_512_256 = self.conv_1x1ir(ir_512)

        rgb_256 = self.ff2rgb(dda_gr, rgb_512_256)
        ir_256  = self.ff2ir(ir, ir_512_256)

        final = self.ff1(rgb_256, ir_256)

        return [final, rgb_256, ir_256]


class FFuse13(nn.Module):
    def __init__(self, in_channel):
        super(FFuse13, self).__init__()
        self.mam = MAM2(in_channel)
        self.dda = DDA(in_channel)
        self.conv_1x1rgb = nn.Conv2d(2 * in_channel, in_channel, kernel_size=1)
        self.conv_1x1ir = nn.Conv2d(2 * in_channel, in_channel, kernel_size=1)
    ...
    def forward(self, x):
        ...
        rgb_256_128 = self.conv_1x1rgb(rgb_256)
        ir_256_128 = self.conv_1x1ir(ir_256)

        rgb_128 = self.ff2rgb(dda_gr, rgb_256_128)
        ir_128 = self.ff2ir(ir, ir_256_128)

        final = self.ff1(rgb_128, ir_128)
