...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            # Update best mAP
            # fi = fitness(np.array(results).reshape(1, -1)) # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            fi = (np.array(results).reshape(1, -1))[0][2] # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            if fi > best_fitness:
                best_fitness = fi
            wandb_logger.end_epoch(best_result=best_fitness == fi)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--cfg', type=str, default='models/my/yolov5s_fusion_dvtod.yaml', help='model.yaml path')
    # parser.add_argument('--cfg', type=str, default='models/my/yolov5s_multi_fusion_1.yaml', help='model.yaml path')
    parser.add_argument('--cfg', type=str, default='models/my/multi_ffusion_1031.yaml', help='model.yaml path')
